...
section_name <- "6a_run_control_lm"
...
no_genes2test <- 10
start_ind <- 0
subset_genes_by_expression <- FALSE
PLOT <- FALSE
...
for (ind in 1:length(args)){
  ...
  if (arg == "--out_date"){ out_date <- args[[ind + 1]] }
  ...
  if (arg == "--incl_unassigned"){incl_unassigned <- as.logical(args[[ind + 1]]) }
}

if (!(exists('out_date'))){out_date <- date}
print(paste("date:", date))
print(paste("out_date:", date))
...
if (ExperimentName == "Replogle"){
  ...
  outdir <- file.path(OutFolder, section_name, out_date)
...
if (ExperimentName == "Magpie"){
  ...
  if (incl_unassigned == T){
    fnms <- grep(list.files(file.path(OutFolder, "5a_combine"), pattern = date, full.names = T), pattern = "control_cells[.]RDS|unassigned_cells[.]RDS", value = T)
    
    seurat_objects <- lapply(fnms, readRDS)
    #inlets_combined <- readRDS(file.path(OutFolder, "5a_combine", paste0(date, "_ALL_control_cells.RDS")))
    print("seurat object loaded")
    genes2test <- rownames(seurat_objects[[1]])[(start_ind + 1):(min(length(rownames(seurat_objects[[1]])), (start_ind+no_genes2test)))]
    normalized_expression <- lapply(seurat_objects, FUN=function(seurat_object){
      as.data.frame(seurat_object@assays$RNA@data[genes2test,])
    }) %>% bind_cols() %>% as.data.frame()
    counts <- lapply(seurat_objects, FUN=function(seurat_object){
      as.data.frame(seurat_object@assays$RNA@counts[genes2test,])
    }) %>% bind_cols() %>% as.data.frame()
    #run lm
    print("cell metadata obtained!")
    data4lm <- lapply(seurat_objects, FUN=function(seurat_object){
      rtn <- data.frame(cell_line = seurat_object$cell_line,
                        inlet = c(seurat_object$orig.ident),
                        nCount_RNA = c(seurat_object$nCount_RNA),
                        percent_MT = c(seurat_object$percent_MT),
                        s.score = c(seurat_object$S.Score),
                        g2m.score = c(seurat_object$G2M.Score)
      )
  ...
  inlets_combined <- readRDS(file.path(OutFolder, "2a_make_sc_seurat_object", paste0(cell_type, "_", screen, "_sc_control_only.RDS")))
  #inlets_combined <- subset(inlets_combined, target_gene_name == NonTargetGeneName)
  genes2test <- rownames(inlets_combined)[(start_ind + 1):(min(length(rownames(inlets_combined)), (start_ind+no_genes2test)))]
  normalized_expression <- inlets_combined@assays$RNA@data[genes2test,]
  #run lm
  print("cell metadata obtained!")
  data4lm <- data.frame(nCount_RNA = c(inlets_combined$nCount_RNA),
                        percent_MT = c(inlets_combined$percent_MT),
                        s.score = c(inlets_combined$S.Score),
                        g2m.score = c(inlets_combined$G2M.Score)
  )
...
control_lm <- foreach (i = 1:length(genes2test)) %dopar% {
  ...
  if (ExperimentName == "Magpie"){
    ...
    y4lm <- as.numeric(normalized_expression[i,])
    fit <- lm(y4lm ~ cell_line + inlet + nCount_RNA + percent_MT + s.score + g2m.score, data = data4lm %>% mutate(y4lm = y4lm))
  } else if (ExperimentName == "Replogle"){
  ...
  } else if (ExperimentName == "Pica" & incl_unassigned == T) {
    
    n_control = dim(counts)[2]
    rtn$n_control <- n_control
    #control_norm_expr
    rtn$control_num_reads <- sum(counts[genes2test[i],])
    rtn$control_norm_expr <- sum(normalized_expression[genes2test[i],])/n_control
    
    rtn$av_log_expression_control <- log(sum(expm1(as.matrix(normalized_expression[genes2test[i], ]))) /n_control + 1/scale_factor, base = lfc_base) 
  ...
  rtn$av_log_expression_control <- log(sum(expm1(as.matrix(normalized_expression))) /n_control + 1/scale_factor, base = lfc_base) 
  ...
  if (scale_factor != 1){
    ...
    if (ExperimentName == "Replogle"){
      fnm = paste0("gene-", gsub(gene_name, pattern = "/", replacement = "-"), "_", cell_type, ".RDS")
    } else {
      fnm = paste0("gene-", gsub(gene_name, pattern = "/", replacement = "-"), ".RDS")
    }
